...
>
  ...
  <div class='resizers'>
    <!-- 给handler添加事件监听，触发 startResize函数更新位置大小 -->
     <!-- @change 事件不需要显式传递 $event -->
  ...
  setup (props, context) {
    ...
    const startMove = (e: MouseEvent) => {
      ...
      const handleMouseUp = (e: MouseEvent) => {
        ...
        // 事件通过nextTick移除了自己
        // 在事件监听中使用nextTick 完成在下一次dom更新时移除自己这个事件
        nextTick(() => {
    ...
    const caculateSize = (direction: ResizeDirection, e: MouseEvent, positions: OriginalPositions) => {
      ...
      // 加上滚动隐藏的内容container.scrollTop
      const topOffset = pageY + container.scrollTop - container.offsetTop
      switch (direction) {
        case 'top-left':
          return {
            width: leftWidth,
    ...
    // 调整大小的函数，在触发handle的moseDown事件时调用，内部添加监听鼠标move和Up的事件，
    // 在监听鼠标up的事件中移除
